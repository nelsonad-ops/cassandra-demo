  ...
  template:
    spec:
      ...
      - name: cassandra-stress
        image: cassandra:{d.version}
        command:
          - "/bin/bash"
          - "-c"
          - '/opt/cassandra/tools/bin/cassandra-stress write no-warmup n={d.ops} cl=ONE -mode native user=<username> password=<password> cql3 connectionsPerHost={d.connections_per_host} -col n=FIXED\(5\) size=FIXED\(64\)  -pop seq={}..{} -node "{d.host}" -rate threads={d.threads} {d.limit} -log file=/opt/cassandra/logs/cassandra-stress.load.data -schema "replication(factor=1)" -errors ignore; cat /opt/cassandra/logs/cassandra-stress.load.data'
        resources:
          limits:
            cpu: {d.cpu}
            memory: {d.memory}
...
def parse():
    ...
    parser.add_argument('--version', default='4.1', help='version of Cassandra to use for cassandra-stress - defaults to 2.3.1', dest='version')
    parser.add_argument('--host', default='cassandra-cluster-client.cassandra.svc', help='ip or dns name of host to connect to - defaults to cassandra-cluster-client.cassandra.svc')
    ...
    parser.add_argument('--nodeselector', default="", help='nodeselector limits cassandra-stress pods to certain nodes. Use as a label selector, eg. --nodeselector role=cassandra')
